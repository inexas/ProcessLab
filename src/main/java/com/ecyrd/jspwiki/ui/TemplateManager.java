...
public class TemplateManager
	 ...
	 */
	public TemplateManager(WikiEngine engine, @SuppressWarnings("unused") Properties properties) {
		super(engine);
	 ...
	 */
	public String findResource(@SuppressWarnings("unused") WikiContext ctx, String template, String name) {
		...
		try {
			...
			while((entry = jarStream.getNextJarEntry()) != null) {
				...
				if(!entry.isDirectory() && name.startsWith(I18NRESOURCE_PREFIX) && name.endsWith(I18NRESOURCE_SUFFIX)) {
					...
					Locale locale = new Locale(name.substring(0, 2), ((name.indexOf("_") == -1) ? "" : name.substring(
					        3, 5)));
	...
	public Map listTimeFormats(PageContext pageContext) {
		...
		if(tfArr.size() == 0) /*
							 * no props found - make sure some default formats
							 * are avail
							 */
		{
			tfArr.add("dd-MMM-yy");
