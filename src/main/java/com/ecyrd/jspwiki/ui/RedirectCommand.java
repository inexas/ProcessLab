...
/**
 * <p>
 * Defines Commands for redirections to off-site special pages. RedirectCommands
 * do not have associated permissions; the {@link #requiredPermission()} method
 * will always return <code>null</code>. When combined with a supplied String
 * url, the {@link #getTarget()} method will return a String, the
 * {@link #getURLPattern()} method will return the supplied target URL, and
 * {@link #getJSP()} method will return the "cleansed" URL.
 * </p>
 * <p>
...
public final class RedirectCommand extends AbstractCommand {
	...
	/**
	 * Creates and returns a targeted Command by combining a URL (as String)
	 * with this Command. The supplied <code>target</code> object must be non-
	 * <code>null</code> and of type String. The URL passed to the constructor
	 * is actually an URL pattern, but it will be converted to a JSP page if it
	 * is a partial URL. If it is a full URL (beginning with
	 * <code>http://</code> or <code>https://</code>), it will be
	 * "passed through" without conversion, and the URL pattern will be
	 * <code>null</code>.
	 * 
	 * @param target
