 ...
 * static. They are <code>final</code>, threadsafe, and immutable. They have no
 * intrinsic idea of the context they are acting in. For example, the static
 ...
 * <li><strong>Targeted commands</strong> "decorate" static commands by scoping
 * a static Command at a specific target such as a WikiPage or GroupPrincipal.
 * Targeted commands are created by calling an existing Command's
 * {@link #targetedCommand(Object)} and supplying the target object.
 ...
 * </p>
 * <ul>
 * <li><strong>PageCommand</strong>: commands for editing, renaming, and viewing
 * pages</li>
 * <li><strong>GroupCommand</strong>: commands for viewing, editing and deleting
 * wiki groups</li>
 ...
 * non-<code>null</code> value. In addition, its {@link #requiredPermission()}
 * method will generally also return a non-<code>null</code> value. It is each
 * implementation's responsibility to construct and store the correct Permission
 * for a given Command and Target. For example, when PageCommand.VIEW is
 * targeted at the WikiPage <code>Main</code>, the Command's associated
 * permission is <code>PagePermission "<em>theWiki</em>:Main", "view".</code>
 * </p>
 * <p>
...
public interface Command {
	 ...
	 * within the JSPWiki webapp; it is not a general HTTP URL. If it exists,
	 * the JSP will be expressed relative to the webapp root, without a leading
	 * slash. This method is guaranteed to return a non-<code>null</code>
	 ...
	 * {@link PageCommand#targetedCommand(Object)}, the resulting Command would
