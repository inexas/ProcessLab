...
public final class PropertyReader {
	 ...
	 * DEFAULT_PROPERTYFILE. {@value #DEFAULT_PROPERTYFILE}
	 */
	 ...
	 * can overwrite the default values and only specify the properties you need
	 * to change in a multiple wiki environment.
	 * <p>
	...
	public static Properties loadWebAppProps(ServletContext context) {
		...
		try {
			...
			final String defaultPagesPath = context.getRealPath("WEB-INF/data");
			if(props.get("jspwiki.fileSystemProvider.pageDir") == null) {
				props.setProperty("jspwiki.fileSystemProvider.pageDir", defaultPagesPath);
			}
			if(props.get("jspwiki.basicAttachmentProvider.storageDir") == null) {
				props.setProperty("jspwiki.basicAttachmentProvider.storageDir", defaultPagesPath);
			}
			if(props.get("jspwiki.workDir") == null) {
	...
	public static final Properties getDefaultProperties() {
		...
		if(in != null) {
			try {
				...
				System.err.println("Unable to load default propertyfile '" + DEFAULT_JSPWIKI_PROPERTIES + "'"
				        + e.getMessage());
			}
		}
	 ...
	 * In property values you can then use the "$x" identifier to use this
	 * variable.
	 * 
	 * For example you could declare a base directory for all your files like
	 * this and use it in all your other property definitions with a "$basedir".
	 * Note that it does not matter if you define the variable before its usage.
	 * 
	 * <pre>
	...
	public static void expandVars(Properties properties) {
		...
		while(propertyList.hasMoreElements()) {
			...
			while(iter.hasNext()) {
				...
				// replace old property value, using the same variabe. If we
				// don't overwrite
				// the same one the next loop works with the original one again
				// and
