 ...
 * configured via settings in <code>processlab.properties</code>.
 * </p>
 * <p>
...
public class JDBCGroupDatabase implements GroupDatabase {
	 ...
	 *            the group to remove
	 * @throws WikiSecurityException
	 *             if the database does not contain the supplied group (thrown
	 *             as {@link NoSuchPrincipalException}) or if the commit did not
	 *             succeed
	 */
	 ...
	 * fail, and throw an <code>IllegalArgumentException</code>, if the proposed
	 * group is the same name as one of the built-in Roles: e.g., Admin,
	 * Authenticated, etc. The database is responsible for setting create/modify
	 * timestamps, upon a successful save, to the Group. The method commits the
	...
	public void initialize(WikiEngine engine, Properties props) throws NoRequiredPropertyException,
		...
		try {
			...
			m_insertGroup = "INSERT INTO " + m_table + " (" + m_name + "," + m_modified + "," + m_modifier + ","
			        + m_created + ","
			        + m_creator + ") VALUES (?,?,?,?,?)";
		...
		try {
			...
			log.warn("JDBCGroupDatabase warning: user database doesn't seem to support transactions. Reason: "
			        + e.getMessage());
			throw new NoRequiredPropertyException(PROP_GROUPDB_DATASOURCE, "JDBCGroupDatabase initialization error: "
			        + e.getMessage());
		} finally {
