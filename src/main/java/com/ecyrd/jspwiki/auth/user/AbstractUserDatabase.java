...
public abstract class AbstractUserDatabase implements UserDatabase {
	 ...
	 * </p>
	 * <p>
	 * When this method creates WikiPrincipals, the Principal containing the
	 * user's full name is marked as containing the common name (see
	 ...
	 * the user database, this method always returns <code>false</code>. If the
	 * user exists, the supplied password is compared to the stored password.
	 * Note that if the stored password's value starts with <code>{SHA}</code>,
	 * the supplied password is hashed prior to the comparison.
	 * 
	 * @param loginName
	 *            the user's login name
	 * @param password
	 *            the user's password (obtained from user input, e.g., a web
	 *            form)
	...
	public boolean validatePassword(String loginName, String password) {
		...
		try {
			...
			// If in the old format and password verified, upgrade the hash to
			// SSHA
			if(verified) {
	 ...
	 * 
	 * @param db
	 *            The database for which the UID should be generated.
	 ...
	 *            the text to hash
	 * @return the result hash
	 * @deprecated this method is retained for backwards compatibility purposes;
	 *             use {@link #getHash(String)} instead
	 */
