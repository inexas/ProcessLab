...
public interface UserDatabase {
	 ...
	 * {@link #rename(String, String)}, {@link #save(UserProfile)} and
	 * {@link #deleteByLoginName(String)} methods are atomic themselves.
	 * 
	 * @throws WikiSecurityException
	 * @deprecated there is no need to call this method because the save, rename
	 *             and delete methods contain their own commit logic
	 */
	 ...
	 * 
	 * @param identifier
	 *            the name of the user to retrieve; this corresponds to value
	 *            returned by the user profile's
	 ...
	 * Principal objects. If the database does not contain any profiles, this
	 ...
	 * database does not contain a user with a unique ID, it throws a
	 * {@link NoSuchPrincipalException}.
	 * 
	 * @param uid
	...
	/**
	 * Factory method that instantiates a new user profile. The
	 * {@link UserProfile#isNew()} method of profiles created using this method
	...
	/**
	 * <p>
	 * Renames a {@link UserProfile} in the user database by changing the
	 * profile's login name. Because the login name is the profile's unique
	 * identifier, implementations should verify that the identifier is "safe"
	 * to change before actually changing it. Specifically: the profile with the
	 * supplied login name must already exist, and the proposed new name must
	 * not be in use by another profile.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param loginName
	 *            the existing login name for the profile
	 * @param newName
	 *            the proposed new login name
	 * @throws NoSuchPrincipalException
	 *             if the user profile identified by <code>loginName</code> does
	 *             not exist
	 * @throws DuplicateUserException
	 *             if another user profile with the proposed new login name
	 *             already exists
	 * @throws WikiSecurityException
	 *             if the profile cannot be renamed for any reason, such as an
	 *             I/O error, database connection failure or lack of support for
	 *             renames.
	 */
	public void rename(String loginName, String newName) throws NoSuchPrincipalException, DuplicateUserException,
	        WikiSecurityException;

	/**
	 * <p>
	 * Saves a {@link UserProfile}to the user database, overwriting the existing
	 * profile if it exists. The user name under which the profile should be
	 * saved is returned by the supplied profile's
	 * {@link UserProfile#getLoginName()} method.
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * Implementations are <em>required</em> to time-stamp the creation or
	 * modification fields of the UserProfile./p>
	 * <p>
	 * This method is intended to be atomic; results cannot be partially
	 * committed. If the commit fails, it should roll back its state
	 * appropriately. Implementing classes that persist to the file system may
