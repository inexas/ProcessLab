 ...
 * <code>Authenticated</code>. Once created, JDBCUserDatabase does not use this
 * column again; it is provided strictly for the convenience of
 ...
 * called. If the database does <em>not</em> support transactions, then changes
 * are made immediately (during the {@link #save(UserProfile)} method), and the
...
public class JDBCUserDatabase extends AbstractUserDatabase {
	...
	public void initialize(WikiEngine engine, Properties props) throws NoRequiredPropertyException {
		...
		try {
			...
			m_renameProfile = "UPDATE " + m_userTable + " SET " + m_loginName + "=?," + m_modified + "=? WHERE "
			        + m_loginName
			        + "=?";
			...
			throw new NoRequiredPropertyException(PROP_DB_DATASOURCE, "JDBCUserDatabase initialization error: "
			        + e.getMessage());
		}
	...
	public void save(UserProfile profile) throws WikiSecurityException {
		...
		try {
			...
			Timestamp ts = new Timestamp(System.currentTimeMillis());
			Date modDate = new Date(ts.getTime());
			java.sql.Date lockExpiry = profile.getLockExpiry() == null ? null : new java.sql.Date(profile
			        .getLockExpiry().getTime());
	 ...
	 * named column's value. This method will also set the UID if it has not yet
	 * been set.
	 * 
	 * @param sql
	 *            the SQL statement that should be prepared; it must have one
	 *            parameter to set (either a String or a Long)
	 * @param index
	 *            the value to match
