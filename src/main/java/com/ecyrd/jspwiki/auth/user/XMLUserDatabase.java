...
/**
 * <p>
 * Manages {@link DefaultUserProfile} objects using XML files for persistence.
 * Passwords are hashed using SHA1. User entries are simple
 * <code>&lt;user&gt;</code> elements under the root. User profile properties
 * are attributes of the element. For example:
 * </p>
 * <blockquote><code>
 * &lt;users&gt;<br/>
 ...
 * </code></blockquote>
 * <p>
 * In this example, the un-hashed password is <code>myP@5sw0rd</code>. Passwords
 * are hashed without salt.
 * </p>
 * 
...
// FIXME: If the DB is shared across multiple systems, it's possible to lose
// accounts
// if two people add new accounts right after each other from different wikis.
public class XMLUserDatabase extends AbstractUserDatabase implements Serializable {
...
//		}
		...
		c_file = new File(pageDir + DEFAULT_USERDATABASE);
	 ...
	 */
	public synchronized void rename(String loginName, String newName) throws NoSuchPrincipalException,
	        DuplicateUserException, WikiSecurityException {
		if(c_dom == null) {
	 ...
	 * &lt;user&gt; element's supplied attribute. This method will also set the
	 * UID if it has not yet been set.
	 * 
	...
	/**
	 * After loading the DOM, this method sanity-checks the dates in the DOM and
	 * makes sure they are formatted properly. This is sort-of hacky, but it
	 * should work.
	 */
