 ...
 * </p>
 * <p>
 * The WikiSession's are stored both in the remote user HttpSession and in the
 * SessionMonitor for the WikeEngine. This class must be configured as a session
 * listener in the web.xml for the wiki web application.
 * </p>
 */
public class SessionMonitor implements HttpSessionListener {
	...
	/**
	 * Returns the instance of the SessionMonitor for this wiki. Only one
	 * SessionMonitor exists per WikiEngine.
	 * 
	 * @param engine
	 *            the wiki engine
	...
	/**
	 * Just looks for a WikiSession; does not create a new one. This method may
	 * return <code>null</code>, <em>and
	...
	/**
	 * <p>
	 * Looks up the wiki session associated with a user's Http session and adds
	 * it to the session cache. This method will return the "guest session" as
	 * constructed by {@link WikiSession#guestSession(WikiEngine)} if the
	 * HttpSession is not currently associated with a WikiSession. This method
	 * is guaranteed to return a non-<code>null</code> WikiSession.
	 * </p>
	 * <p>
	...
	/**
	 * Removes the wiki session associated with the user's HttpSession from the
	 * session cache.
	 * 
	 * @param session
	 *            the user's HTTP session
	 ...
	 * </p>
	 * <p>
	 * To obtain the list of current WikiSessions, we iterate through our
	 * session Map and obtain the list of values, which are WikiSessions wrapped
	 * in {@link java.lang.ref.WeakReference} objects. Those
	 * <code>WeakReference</code>s whose <code>get()</code> method returns non-
	 * <code>null</code> values are valid sessions.
	 * </p>
	 * 
	 ...
	 *            the event type
	 * @param principal
	 *            the user principal associated with this session
	 * @param session
	 *            the wiki session
	 * @since 2.4.75
	 */
	...
	/**
	 * Removes the user's WikiSession from the internal session cache when the
	 * web container destoys an HTTP session.
	 * 
	 * @param se
	 *            the HTTP session event
	 */
