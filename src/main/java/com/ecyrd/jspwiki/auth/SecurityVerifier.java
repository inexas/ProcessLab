...
public final class SecurityVerifier {
	...
	/**
	 * Constructs a new SecurityVerifier for a supplied WikiEngine and
	 * WikiSession.
	 * 
	 * @param engine
	 *            the wiki engine
	 ...
	 * 
	 * @return the roles parsed from <code>web.xml</code>, or a zero-length
	 *         array
	 * @throws WikiException
	 ...
	 * 
	 * @throws WikiException
	 *             if the web authorizer cannot verify the roles
	 */
		...
		for(Principal principal : m_policyPrincipals) {
			if(principal instanceof Role) {
				...
				if(!Role.isBuiltInRole(role) && !isContainerRole) {
					m_session.addMessage(ERROR_ROLES, "Role '" + role.getName()
					        + "' is defined in security policy but not in web.xml.");
					missing = true;
				}
			}
		}
	...
	/**
	 * Verifies that the group datbase was initialized properly, and that user
	 * add and delete operations work as they should.
	 */
	...
	protected final void verifyJaas() {
		...
		// Verify that the specified JAAS moduie corresponds to a class we can
		// load successfully.
	 ...
	 * associated File object if it exists. This method adds messages with the
	 * topic prefix {@link #ERROR} and {@link #INFO} as appropriate, with the
	 * suffix matching the supplied property.
	 * 
	 * @param property
	 ...
	 * valid if value of the local policy (at
	 * <code>WEB-INF/jspwiki.policy</code> resolves to an existing file, and the
	 * policy file contained therein represents a valid policy.
	 */
	...
	/**
	 * Verifies that the user datbase was initialized properly, and that user
	 * add and delete operations work as they should.
	 */
