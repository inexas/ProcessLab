 ...
 * information is stored in the <code>jspwiki.workDir</code>, under the
 * directory {@value #COOKIE_DIR}. For security purposes it is a very, very good
 * idea to prevent access to this directory by everyone except the web server
 * process; otherwise people having read access to this directory may be able to
 * spoof other users.
 * <p>
 ...
 * </p>
 * <ol>
 * <li>{@link HttpRequestCallback}- supplies the cookie, which should contain an
 * unique id for fetching the UID.</li>
...
public class CookieAuthenticationLoginModule extends AbstractLoginModule {
	...
	/**
	 * The directory name under which the cookies are stored. The value is
	 * {@value} .
	 */
	 ...
	 * computer. The value is {@value} . The default expiry time is 14 days.
	 */
	...
	/**
	 * Clears away the login cookie, and removes the uid-username mapping file
	 * as well.
	 * 
	 * @param engine
	 *            WikiEngine
	...
	/**
	 * Goes through the cookie directory and removes any obsolete files. The
	 * scrubbing takes place one day after the cookie was supposed to expire.
	 * However, if the user has logged in during the expiry period, the expiry
	 * is reset, and the cookie file left here.
	 * 
	 * @param days
	 * @param cookieDir
	 */
