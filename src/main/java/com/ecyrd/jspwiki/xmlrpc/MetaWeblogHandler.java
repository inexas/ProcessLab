 ...
 * <P>
 * JSPWiki does not support categories, and therefore we always return an empty
 * list for getCategories(). Note also that this API is not suitable for general
 * Wiki editing, since JSPWiki formats the entries in a wiki-compatible manner.
 * And you cannot choose your page names either. Since 2.1.94 the entire
 * MetaWeblog API is supported.
 * 
 * @since 2.1.7
 */
...
public class MetaWeblogHandler
	...
	/**
	 * Does a quick check against the current user and does he have permissions
	 * to do the stuff that he really wants to.
	 * <p>
	...
	private void checkPermissions(WikiPage page,
		...
		try {
			...
			if(amm.login(m_context.getWikiSession(), username, password)) {
				if(!mgr.checkPermission(m_context.getWikiSession(), PermissionFactory.getPagePermission(page,
				        permission))) {
	...
	/**
	 * JSPWiki does not support categories, therefore JSPWiki always returns an
	...
	public String newPost(String blogid,
	        ...
	        String password,
	        Hashtable content,
	        @SuppressWarnings("unused") boolean publish)
	        throws XmlRpcException {
	...
	/**
	 * Creates an attachment and adds it to the blog. The attachment is created
	 * into the main blog page, not the actual post page, because we do not know
	 * it at this point.
	 * 
	 * @param blogid
	 *            The id of the blog.
	...
	/**
	 * Allows the user to edit a post. It does not allow general editability of
	 * wiki pages, because of the limitations of the metaWeblog API.
	 */
	...
	/**
	 * Gets the text of any page. The title of the page is parsed (if any is
	 * provided).
	 */
	Hashtable getPost(@SuppressWarnings("unused") String postid,
	        String username,
	        String password)
	        throws XmlRpcException {
