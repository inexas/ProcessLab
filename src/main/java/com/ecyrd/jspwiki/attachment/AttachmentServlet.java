...
public class AttachmentServlet
	...
	private boolean isTypeAllowed(String theName) {
		if(theName == null || theName.length() == 0)
			return false;

		final String name = theName.toLowerCase();
	...
	protected boolean executeUpload(WikiContext context, InputStream data,
		...
		try {
			...
			// properly :
			throw new RedirectException(context.getBundle(InternationalizationManager.CORE_BUNDLE).getString(
		...
		if(m_engine.getAuthorizationManager().checkPermission(context.getWikiSession(),
			...
			try {
				...
				throw new ProviderException(context.getBundle(InternationalizationManager.CORE_BUNDLE).getString(
				        pe.getMessage()));
			}
