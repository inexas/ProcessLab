...
public class XHtmlElementToWikiTranslator {
	 ...
	 * 
	 * @param base
	 *            The base element from which to start translating.
	 * @throws IOException
	 *             If reading of the DOM tree fails.
	...
	private void print(Object element) throws IOException, JDOMException {
		if(element instanceof Text) {
			...
			if("imageplugin".equals(base.getAttributeValue("class"))) {
				...
				// only print the children if the div's class="wikiform", but
				// not the div itself.
				printChildren(base);
			} else {
				boolean bold = false;
				...
				// Only get the styles if it's not a link element. Styles for
				// link elements are
		...
		for(Iterator i = base.getContent().iterator(); i.hasNext();) {
			...
			if(c instanceof Element) {
				...
				if(n.equals("h1")) {
					...
					if(e.getContentSize() != 0) // we don't want to print empty
												// elements: <p></p>
					{
						m_out.println();
						print(e);
					...
					if(m_preStack.isPreMode()) {
						...
						//
						// To beautify the generated wiki markup, we print a
						// newline character after a linebreak.
						// It's only safe to do this when the parent element is
						// a <p> or <div>; when the parent
						// element is a table cell or list item, a newline
						// character would break the markup.
						// We also check that this isn't being done inside a
						// plugin body.
						//
								...
								if(ref != null) {
									if(ref.startsWith("#")) {
										...
										if(!e.getTextTrim().equalsIgnoreCase(ref)) {
											...
											// If the ref has the same value as
											// the text and also if there
											// are attributes, then just print:
											// [ref|ref|attributes] .
											m_out.print("|" + ref + "|");
											String augmentedWikiLink = augmentedWikiLinkMapToString(augmentedWikiLinkAttributes);
											m_out.print(augmentedWikiLink);
					...
					print(e);
					m_out.print("%%");
					// NOTE: don't print a space before or after the double
					// percents because that can break words into two.
					// For example: %%(color:red)ABC%%%%(color:green)DEF%% is
					// different from %%(color:red)ABC%% %%(color:green)DEF%%
					...
					// The following line assumes that the XHTML has been
					// "pretty-printed"
					...
					// only print a newline if this <li> element is not the last
					// item within a sublist.
					if(!sublistItem || !lastListItem) {
						m_out.println();
					}
				} else if(n.equals("pre")) {
					m_out.print("\n{{{\n"); // start JSPWiki "code blocks" on
											// its own line
					m_preStack.push();
					print(e);
					m_preStack.pop();

					// print a newline before the closing braces for aesthetics
					// and a newline after it
					// to avoid breaking any subsequent wiki markup that
					// follows.
					m_out.print("\n}}}\n");
					...
					m_preStack.pop();
					m_out.print("}}");
					// NOTE: don't print a newline after the closing brackets
					// because if the Text is inside
					// a table or list, it would break it if there was a
					// subsequent row or list item.
					...
					// remove the hidden input where name="formname" since a new
					// one will be generated again when the xhtml is rendered.
					...
					if(name != null) {
						// remove the "nbf_" that was prepended since new one
						// will be generated again when the xhtml is rendered.
					...
					// If this <option> element isn't the second child element
					// within the parent <select>
					// element, then we need to print a semicolon as a
					// separator. (The first child element
					// is expected to be a newline character which is at index
					// of 0).
					if(base.indexOf(e) != 1) {
						m_out.print(";");
					}
	...
	private Map getStylePropertiesLowerCase(Element base) throws IOException {
		...
		if(n.equals("p") || n.equals("div")) {
			...
			if(align != null) {
				// only add the value of the align attribute if the text-align
				// style didn't already exist.
