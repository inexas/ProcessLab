...
public class WikiEngine implements Serializable {
     ...
     * 
     */
    private static final long serialVersionUID = 2460092777889340168L;
	...
	public static synchronized WikiEngine getInstance(ServletContext context, Properties props)
		...
		if(engine == null) {
			...
			try {
				final Properties p = props == null ? PropertyReader.loadWebAppProps(context) : props;
				engine = new WikiEngine(context, appid, p);
	...
	private void initialize(Properties props)
		...
		try {
			...
			m_authenticationManager = (AuthenticationManager)ClassUtil.getMappedObject(AuthenticationManager.class
			        .getName());
			m_authorizationManager = (AuthorizationManager)ClassUtil.getMappedObject(AuthorizationManager.class
			        .getName());
			...
			getFilterManager().addPageFilter(schedulerFilter, -1003);	// Why 1004? I don't know
			
			// KW start the DiskDataCache...
			new DiskDataCache(this, props);
			Scheduler.initialize(this, props);

			PageLogger.initialize(props);
		}
	...
	public void initReferenceManager() throws WikiException {
		try {
			...
			if(m_referenceManager == null) {
				...
				schedulerFilter = new SchedulerFilter();
				schedulerFilter.initialize(this, pages);
			}
	...
	public String getViewURL(String pageName) {
		final String pn = pageName == null ? getFrontPage() : pageName;
		return getURLConstructor().makeURL(WikiContext.VIEW, pn, true, null);
	}
	...
	public String getURL(String context, String pageName, String params, boolean absolute) {
		final String pn = pageName == null ? getFrontPage() : pageName;
		return m_urlConstructor.makeURL(context, pn, absolute, params);
	}
	...
	/**
	 * Returns the {@link com.ecyrd.jspwiki.workflow.WorkflowManager} associated
	 ...
	 */
	public String getText(@SuppressWarnings("unused") WikiContext context, WikiPage page) {
	...
	public String textToHTML(WikiContext context, String pagedata) {
		...
		try {
			final String pd =
				runFilters ? m_filterManager.doPreTranslateFiltering(context, pagedata) : pagedata;
			result = m_renderingManager.getHTML(context, pd);
	...
	private String textToHTML(WikiContext context,
		...
		try {
			...
			final String pd =
				runFilters ? m_filterManager.doPreTranslateFiltering(context, pagedata) : pagedata;
			MarkupParser mp = m_renderingManager.getParser(context, pd);
	 ...
	 * a {@link com.ecyrd.jspwiki.workflow.Decision} in the approver's workflow
	 * inbox and throw a
	 * {@link com.ecyrd.jspwiki.workflow.DecisionRequiredException}. If the
	 ...
	 *             {@link com.ecyrd.jspwiki.filters.SpamFilter} may also throw a
	 *             {@link com.ecyrd.jspwiki.filters.RedirectException}.
	 */
	...
	public AclManager getAclManager() {
		if(m_aclManager == null) {
			try {
				...
				// XXX: I am not sure whether this is the right call
	...
	/**
	 * This is a nascent but important function. The idea is it allows callers
	 * to query the engine for contexts, pages, parts of pages or structured 
	 * data etc., given a path. This will grow and become a key function in 
	 * ProcessLab!
	 * 
	 * In the meantime we handle a simplified path
	 *   
	 * @param key in the form <PageName>=<ValueName>
	 * @return
	 */
	public String getNamedValue(String path) {
		final String[] strings = path.split("=");
		assert strings.length == 2;
		final String pageName = strings[0];
		final String name = strings[1];
		final NameValueMap map = new NameValueMap(this, pageName);
