...
/**
 * Provides a caching attachment provider. This class rests on top of a real
 * provider class and provides a cache to speed things up. Only the Attachment
 * objects are cached; the actual attachment contents are fetched always from
 * the provider.
 * 
 * @since 2.1.64.
 */

// FIXME: Do we need to clear the cache entry if we get an NRE and the
// attachment is not there?
// FIXME: We probably clear the cache a bit too aggressively in places.
// FIXME: Does not yet react well to external cache changes. Should really use
// custom
...
public class CachingAttachmentProvider
	 ...
	 * This provides for quickly giving recently changed attachments (for the
	...
	public List listAllChanged(Date timestamp)
		...
		List all = null;
		//
		// we do a one-time build up of the cache, after this the cache is
		// updated for every attachment add/delete
		if(m_gotall == false) {
	...
	/**
	 * Simply goes through the collection and attempts to locate the given
	 ...
	 * something like that) Idea copied from CacheItemCollector The cache is
	 * used to speed up the getRecentChanges function
	 * 
	 * @author Harry Metske
	 * @since 2.5
	 */
