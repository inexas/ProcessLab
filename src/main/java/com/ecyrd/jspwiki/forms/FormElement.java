...
public abstract class FormElement
	...
	/**
	 * In order to make the form-to-handler parameter transfer easier, we prefix
	 * all user-specified FORM element names with HANDLERPARAM_PREFIX the HTML
	 * output. This lets us differentiate user-defined FormHandler parameters
	 * from Form parameters. The submit handler must then use
	...
	/**
	 * The submit servlet may decide to store a FormInfo with user-entered form
	 * values in the Session. It should use this name, and this class checks for
	 * it to pre-fill fields from a previous form submit.
	 */
	...
	/**
	 * Plugin parameter, mandatory in output element, indicates WikiPlugin to
	 * use to handle form submitted data.
	 */
	...
	/**
	 * If set to 'handler' in output element, the handler plugin is called even
	 * on first invocation (no submit). The plugin can then place values into
	 * its parameter map, and these are seen by subsequent Form elements. (Store
	 * a value in the plugin with the same key as an input element, and the
	 * value will be visible in the initial form.)
	 */
	 ...
	 * 
	 * @param ctx
	 *            The Context to store it in
	 * @param info
	 *            The FormInfo to store.
	 */
	...
	/**
	 * Attempts to retrieve information on the currently handled form from the
	 * WikiContext.
	 * 
	 * @param ctx
	 *            The Context
