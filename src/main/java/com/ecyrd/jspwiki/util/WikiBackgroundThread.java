 ...
 * {@link WikiEngineEvent#SHUTDOWN} event, it terminates itself. Subclasses of
 * this method need only implement the method {@link #backgroundTask()}, instead
 * of the normal {@link Thread#run()}, and provide a constructor that passes the
 * WikiEngine and sleep interval. This class is thread-safe.
 * 
...
public abstract class WikiBackgroundThread extends Thread implements WikiEventListener {
	 ...
	 *            the wiki engine
	 * @param sleepInterval
	 *            the interval between invocations of the thread's
	...
	/**
	 * Listens for {@link com.ecyrd.jspwiki.event.WikiEngineEvent#SHUTDOWN} and,
	 * if detected, marks the thread for death.
	 * 
	 * @param event
	 *            {@inheritDoc}
	...
	/**
	 * Runs the background thread's {@link #backgroundTask()} method at the
	 * interval specified at construction. The thread will initially pause for a
	 * full sleep interval before starting, after which it will execute
	 * {@link #startupTask()}. This method will cleanly terminate the thread if
	 * it has previously been marked as dead, before which it will execute
	 * {@link #shutdownTask()}. If any of the three methods return an exception,
	 * it will be re-thrown as a {@link com.ecyrd.jspwiki.InternalWikiException}
	 * .
	 * 
