 ...
 * <li><b>format</b> - (full|compact) : if "full", then display a long version
 * with all possible info. If "compact", then be as compact as possible.</li>
...
public class RecentChangesPlugin extends AbstractReferralPlugin implements WikiPlugin {
		...
		if(changes != null) {
			...
			for(Iterator i = changes.iterator(); i.hasNext();) {
				...
				if(pageref instanceof Attachment) {
					...
					linkel
					        .addElement(new img().setSrc(context
					                .getURL(WikiContext.NONE, "images/attachment_small.png")));
				...
				if(pageref instanceof Attachment) {
					...
					infocol.addElement(new a(context.getURL(WikiContext.DIFF, pageref.getName(), "r1=-1"), tfmt
					        .format(lastmod)));
				...
				if(showAuthor) {
					...
					if(author != null) {
						...
						authorinfo.addElement(context.getBundle(InternationalizationManager.CORE_BUNDLE).getString(
				...
				// Revert note
				/*
				 * if( context.hasAdminPermissions() ) { row.addElement( new
				 * td("Revert") ); }
				 */
			}
	...
	// XXX: Ideally the default behavior should be to return the default format
	// for the default
