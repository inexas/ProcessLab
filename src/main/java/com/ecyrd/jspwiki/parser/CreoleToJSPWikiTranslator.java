...
/**
 * <p>
 * Translates Creole markp to JSPWiki markup. Simple translator uses regular
 * expressions. See http://www.wikicreole.org for the WikiCreole spec.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * <b>WARNING</b>: This is an experimental feature, and known to be broken. Use
 * at your own risk.</o>
 * 
...
public class CreoleToJSPWikiTranslator {
	...
	// private static final String CREOLE_LINEBREAKS =
	// "([^\\s\\\\])(\r\n|\r|\n)+(?=[^\\s\\*#])";
	...
	/* XXX Is it possible to use just protect :// ? */
	...
	public String translate(Properties wikiProps, final String content) {
		...
		/*
		 * // BROKEN, breaks on different platforms. String tmp =
		 * wikiProps.getProperty("creole.blogLineBreaks"); if (tmp != null) { if
		 * (tmp.trim().equals("true")) blogLineBreaks = true; }
		 */
		String imagePlugin = wikiProps.getProperty("creole.imagePlugin.name");

		String result = content;
		//
		// Breaks on OSX. It is never a good idea to tamper with the linebreaks.
		// JSPWiki always
		...
		/*
		 * if (blogLineBreaks) { result = translateElement(result,
		 * CREOLE_LINEBREAKS, JSPWIKI_LINEBREAKS); }
		 */
