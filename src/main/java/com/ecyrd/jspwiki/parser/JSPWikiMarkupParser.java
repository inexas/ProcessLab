...
public class JSPWikiMarkupParser
	...
	private String makeSectionTitle(String title) {
		...
		try {
			...
			throw new InternalWikiException("CleanTranslator not working as expected, when cleaning title"
			        + e.getMessage());
		}
			...
			try {
				...
				if(!m_wysiwygEditorMode) {
					...
					return addElement(makeError(MessageFormat.format(
					        rbPlugin.getString("plugin.error.insertionfailed"), args)));
				}
			}
			...
			if(VariableManager.isVariableLink(linktext)) {
				...
				if(m_wysiwygEditorMode) {
					...
					if(urlReference != null) {
						...
						Object[] args = { extWiki };
						addElement(makeError(MessageFormat.format(rb.getString("markupparser.error.nointerwikiref"),
						        args)));
					}
				}
	 ...
	 * @throws IOException
	 */
	private Element handleDiv(@SuppressWarnings("unused") boolean newLine)
	        throws IOException {
		int ch = nextToken();
		Element el = null;
	...
	private void closeHeadings() {
		if(m_lastHeading != null && !m_wysiwygEditorMode) {
			...
			addElement(new Element("a").setAttribute("class", "hashlink").setAttribute("href",
			        "#" + m_lastHeading.m_titleAnchor).setText("#"));
			m_lastHeading = null;
		}
		popElement("h2");
		popElement("h3");
