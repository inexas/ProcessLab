 ...
 * context, then the plugin is rendered as WikiMarkup. This allows an HTML
 * editor to work without rendering the plugin each time as well.
 * <p>
 * If RenderingManager.VAR_EXECUTE_PLUGINS is set to Boolean.FALSE, then the
...
public class PluginContent extends Text {
	 ...
	 * invokes the plugin and returns its contents. If there is no Document yet,
	 * only returns the plugin name itself.
	 * 
	...
	public String getText() {
		...
		if(doc == null) {
			//
			// This element has not yet been attached anywhere, so we simply
			// assume there is
			// no rendering and return the plugin name. This is required e.g.
			// when the
			// paragraphify() checks whether the element is empty or not. We
			// can't of course
			// know whether the rendering would result in an empty string or
			// not, but let us
			// assume it does not.
			//
		...
		try {
			//
			// Determine whether we should emit the actual code for this plugin
			// or
			// whether we should execute it. For some plugins we always execute
			// it,
			// since they can be edited visually.
			//
			// FIXME: The plugin name matching should not be done here, but in a
			// per-editor resource
