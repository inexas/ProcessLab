 ...
 * <p>
 * Adding a PageEventFilter to the FilterManager will automatically attach an
 * event delegate with the WikiEventManager to provide for firing and listener
 * management. All that remains is then adding the listener to the filter via
 * the WikiEventManager. This is quite simple:
 * </p>
 * 
 * <pre>
...
public class PageEventFilter extends BasicPageFilter {
	 ...
	 * TranslatorReader, so anything you are seeing here is translated content.
	 ...
	 * registered listeners. Only <tt>PAGE_LOCK</tt> and <tt>PAGE_UNLOCK</tt>
	 * event types will fire an event; other event types are ignored.
	 * 
