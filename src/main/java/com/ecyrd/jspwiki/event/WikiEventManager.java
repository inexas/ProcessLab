 ...
 * listeners. An "event source" is the object delegating its event handling to
 * an inner delegating class supplied by this manager. The class being serviced
 * is considered a "client" of the delegate. The WikiEventManager operates
 * across any number of simultaneously-existing WikiEngines since it manages all
 * delegation on a per-object basis. Anything that might fire a WikiEvent (or
 * any of its subclasses) can be a client. </p>
 * 
 ...
 * {@link #fireEvent(Object,WikiEvent)} method, where the Object is the client.
 * Prior to instantiating the event object, the client can call
 * {@link #isListening(Object)} to see there are any listeners attached to its
 * delegate.
 * </p>
 * 
 ...
 * </pre>
 * 
 * If you only have a reference to the listener, the following method will
 * remove it from any clients managed by the WikiEventManager:
 * 
 * <pre>
 ...
 * <p>
 * Using one manager for all events processing permits consolidation of all
 * event listeners and their associated methods in one place rather than having
 * them attached to specific subcomponents of an application, and avoids a great
 * deal of event-related cut-and-paste code. Convenience methods that call the
 * WikiEventManager for event delegation can be written to maintain existing
 * APIs.
 * </p>
 * <p>
 * For example, an existing <tt>fireEvent()</tt> method might look something
 * like this:
 * </p>
 * 
 * <pre>
 ...
 * which are created even when no listener exists for them. In a busy wiki with
 * many users unused/unnecessary event creation could be considerable. Another
 * advantage is that in addition to the iterator, there must be code to support
 * the addition and remove of listeners. The above could be replaced with the
 * below code (and with no necessary local support for adding and removing
 * listeners):
 * </p>
 * 
 * <pre>
 ...
 * particularly designed for embedded applications that need to be able to
 * listen for the instantiation of an Object, by maintaining a cache of
 * client-less WikiEvent sources that set their client upon being popped from
 * the cache. Each time any of the methods expecting a client parameter is
 * called with a null parameter it will preload an internal cache with a
 * client-less delegate object that will be popped and returned in preference to
 * creating a new object. This can have unwanted side effects if there are
 * multiple clients populating the cache with listeners. The only check is for a
 * Class match, so be aware if others might be populating the client-less cache
 * with listeners.
 * </p>
 * <h3>Listener lifecycle</h3>
 * <p>
 * Note that in most cases it is not necessary to remove a listener. As of
 * 2.4.97, the listeners are stored as WeakReferences, and will be automatically
 * cleaned at the next garbage collection, if you no longer hold a reference to
 * them. Of course, until the garbage is collected, your object might still be
 * getting events, so if you wish to avoid that, please remove it explicitly as
 * described above.
 * </p>
 * 
...
public final class WikiEventManager {
	 ...
	 * 
	 * If <tt>client</tt> is a reference to the WikiEventManager class itself
	 * and the compile-time flag {@link #c_permitMonitor} is true, this attaches
	 ...
	 * <tt>client</tt> as a reference to this class and the <tt>listener</tt>
	 * parameter as null, or
	 ...
	 * 
	 * @param client
	 *            the client of the event source
	 * @param listener
	 *            the event listener
	 * @return true if the listener was added (i.e., it was not already in the
	 *         list and was added)
	 */
	 ...
	 * for the supplied client. If there are no attached listeners, returns an
	 * empty Iterator rather than null. Note that this will create a delegate
	 * for the client if it did not exist prior to the call.
	 * 
	 * <h3>NOTE</h3>
	 * <p>
	 ...
	 * @param client
	 *            the client of the event source
	 * @return an unmodifiable Set containing the WikiEventListeners attached to
	 *         the client
	 * @throws java.lang.UnsupportedOperationException
	 *             if any attempt is made to modify the Set
	 */
	 ...
	 * managed by this WikiEventManager. Since this is a one-to-one relation,
	 * the first match will be returned upon removal; a true return value
	 * indicates the WikiEventListener was found and removed.
	 * 
	 * @param listener
	 *            the event listener
	 * @return true if the listener was found and removed.
	...
	/**
	 * Returns a WikiEventDelegate for the provided client Object. If the
	 * parameter is a class reference, will generate and return a client-less
	 * WikiEventDelegate. If the parameter is not a Class and the delegate cache
	 * contains any objects matching the Class of any delegates in the cache,
	 * the first Class-matching delegate will be used in preference to creating
	 * a new delegate. If a null parameter is supplied, this will create a
	 * client-less delegate that will attach to the first incoming client (i.e.,
	 * there will be no Class-matching requirement).
	 * 
	 * @param client
	 *            the client Object, or alternately a Class reference
	 * @return the WikiEventDelegate.
	 */
	private WikiEventDelegate getDelegateFor(Object client) {
		synchronized(m_delegates) {
			if(client == null || client instanceof Class) // then preload the
														  // cache
			{
																	...
																	// added
				{
					...
					if(delegate.getClientClass() == null
						...
						// we have a hit, so use it, but only on a client we
						// haven't seen before
	 ...
	 * Classes that generate events can obtain an instance of this class from
	 * the WikiEventManager and delegate responsibility to it. Interaction with
	 * this delegating class is done via the methods of the
	 * {@link WikiEventDelegate} API.
	 * 
	 * @author Murray Altheim
	 * @since 2.4.20
	 */
	private static final class WikiEventDelegate {
		...
		/**
		 * Constructor for an WikiEventDelegateImpl, provided with the client
		 * Object it will service, or the Class of client, the latter when used
		 * to preload a future incoming delegate.
		 */
		...
		/**
		 * Returns the class of the client-less delegate, null if this delegate
		 * is attached to a client Object.
		 */
		...
		/**
		 * Return an unmodifiable Set containing the WikiEventListeners of this
		 * WikiEventDelegateImpl. If there are no attached listeners, returns an
		 * empty Set rather than null.
		 * 
		 * @return an unmodifiable Set containing this delegate's
		 *         WikiEventListeners
		 * @throws java.lang.UnsupportedOperationException
		 *             if any attempt is made to modify the Set
		 */
		...
		/**
		 * Adds <tt>listener</tt> as a listener for events fired by the
		 * WikiEventDelegate.
		 * 
		 * @param listener
		 *            the WikiEventListener to be added
		 ...
		 * 
		 * @param listener
		 *            the WikiEventListener to be removed
		 * @return true if the listener was removed (i.e., it was actually in
		...
		/**
		 * Returns true if there are one or more listeners registered with this
		 * instance.
		 */
	...
	private static class WikiEventListenerComparator implements Comparator<WikiEventListener> {
		// XXX: This method is a critical performance bottleneck
