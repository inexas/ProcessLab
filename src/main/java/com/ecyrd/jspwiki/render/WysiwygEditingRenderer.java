...
/**
 * Implements a WikiRendered that outputs XHTML in a format that is suitable for
 * use by a WYSIWYG XHTML editor.
 * 
 * @author David Au
 * @since 2.5
 */
			...
			if(childElement instanceof Element) {
				...
				if(elementName.equals(A_ELEMENT)) {
					if(classAttr != null) {
						...
						// Get the url for wiki page link - it's typically
						// "Wiki.jsp?page=MyPage"
						// or when using the ShortURLConstructor option, it's
						// "wiki/MyPage" .
						...
						if(classValue.equals(WIKIPAGE)
							...
							// Remove the leading url string so that users will
							// only see the
							// wikipage's name when editing an existing wiki
							// link.
							// For example, change "Wiki.jsp?page=MyPage" to
							// just "MyPage".
							String newHref = hrefAttr.getValue().substring(wikiPageLinkUrl.length());

							// Convert "This%20Pagename%20Has%20Spaces" to
							// "This Pagename Has Spaces"
							...
							if(titleAttr != null) {
								// remove the title since we don't want to
								// eventually save the default undefined page
								// title.
								titleAttr.detach();
							}
