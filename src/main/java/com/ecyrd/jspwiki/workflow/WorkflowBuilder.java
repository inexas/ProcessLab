...
/**
 * Factory class that creates common Workflow instances such as a standard
 * approval workflow.
 * 
...
public final class WorkflowBuilder {
	 ...
	 * WorkflowBuilder exists for a given engine.
	 * 
	 * @param engine
	 *            the wiki engine
	 ...
	 * {@link com.ecyrd.jspwiki.auth.authorize.Group} or
	 * {@link com.ecyrd.jspwiki.auth.authorize.Role} before running a Task.
	 * </p>
	 * <p>
	 ...
	 * executed. If the request is denied, a {@link SimpleNotification} with a
	 * message corresponding to the <code>rejectedMessage</code> message key
	 * will be placed in the submitter's workflow queue.
	 * </p>
	 * <p>
	 * To help approvers determine how to make the Decision, callers can supply
	 * an array of Fact objects to this method, which will be added to the
	 * Decision in the order they appear in the array. These items will be
	 * displayed in the web UI. In addition, the value of the first Fact will
	 * also be added as the third message argument for the workflow (the first
	 * two are always the submitter and the approver). For example, the
	 * PageManager code that creates the "save page approval" workflow adds the
	 * name of the page as its first Fact; this results in the page name being
	 ...
	 * @param prepTask
	 *            the initial task that should run before the Decision step is
	 *            processed. If this parameter is <code>null</code>, the
	 *            Decision will run as the first Step instead
	 * @param decisionKey
	 *            the message key in <code>default.properties</code> that
	 *            contains the text that will appear in approvers' workflow
	 *            queues indicating they need to make a Decision; for example,
	 *            <code>decision.saveWikiPage</code>. In the i18n message bundle
	 *            file, this key might return text that reads
	 *            "Approve page &lt;strong&gt;{2}&lt;/strong&gt;"
	 * @param facts
	 *            an array of {@link Fact} objects that will be shown to the
	 *            approver to aid decision-making. The facts will be displayed
	 *            in the order supplied in the array
	 * @param completionTask
	 *            the Task that will run if the Decision is approved
	 * @param rejectedMessageKey
	 *            the message key in <code>default.properties</code> that
	 *            contains the text that will appear in the submitter's workflow
	 *            queue if request was not approved; for example,
	 *            <code>notification.saveWikiPage.reject</code>. In the i18n
	 *            message bundle file, this key might might return text that
	 *            reads
	 *            "Your request to save page &lt;strong&gt;{2}&lt;/strong&gt; was rejected."
	 *            If this parameter is <code>null</code>, no message will be
	 *            sent
	 * @return the created workflow
	 * @throws WikiException
	 *             if the name of the approving user, Role or Group cannot be
	 *             determined
	 */
	public Workflow buildApprovalWorkflow(Principal submitter,
		...
		// If Decision not required, just run the prep + approved tasks in
		// succession
		else {
			// Set the first step
			if(prepTask == null) {
