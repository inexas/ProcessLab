...
/**
 * Abstact superclass that provides a complete implementation of most Step
 * methods; subclasses need only implement {@link #execute()} and
 * {@link #getActor()}.
 * 
...
public abstract class AbstractStep implements Step {
     ...
     * 
     */
    private static final long serialVersionUID = 3452840039687436970L;
	...
	/**
	 * Protected constructor that creates a new Step with a specified message
	 * key. After construction, the protected method
	 ...
	 *            type, plus a period (<em>e.g.</em>, <code>task.</code> and
	 *            <code>decision.</code>).
	 */
	...
	public final synchronized void setOutcome(Outcome outcome) {
		...
		if(!m_successors.containsKey(outcome)) {
			if(!Outcome.STEP_CONTINUE.equals(outcome) &&
				...
				throw new IllegalArgumentException("Outcome " + outcome.getMessageKey()
				        + " is not supported for this Step.");
			}
		}
