 ...
 * </p>
 * <h2>Workflow lifecycle</h2> A Workflow's state (obtained by
 * {@link #getCurrentState()}) will be one of the following: </p>
 * <ul>
 ...
 * encountered errors, and should abort the Step <em>and</em> the Workflow as a
 * whole. When this happens, the Workflow will set the current Step's Outcome to
 * {@link Outcome#STEP_ABORT} and invoke the Workflow's {@link #abort()} method.
 * The Step's processing errors, if any, can be retrieved by
 * {@link Step#getErrors()}.</li>
 * </ul>
 * <li>The Outcome of the <code>execute</code> method also affects what happens
 * next. Depending on the result (and assuming the Step did not abort), the
 * Workflow will either move on to the next Step or put the Workflow into the
 * {@link Workflow#WAITING} state:</li>
 * <ul>
 * <li>If the Outcome denoted "completion" (<em>i.e.</em>, its
 * {@link Step#isCompleted()} method returns <code>true</code>) then the Step is
 * considered complete; the Workflow looks up the next Step by calling the
 * current Step's {@link Step#getSuccessor(Outcome)} method. If
 * <code>successor()</code> returns a non-<code>null</code> Step, the return
 * value is marked as the current Step and added to the Workflow's Step history.
 * If <code>successor()</code> returns <code>null</code>, then the Workflow has
 * no more Steps and it enters the {@link #COMPLETED} state.</li>
 * <li>If the Outcome did not denote "completion" (<em>i.e.</em>, its
 * {@link Step#isCompleted()} method returns <code>false</code>), then the Step
 * still has further work to do. The Workflow enters the {@link #WAITING} state
 * and stops further processing until a caller restarts it.</li>
 * </ul>
 * </ul> </p>
 * <p>
 * The currently executing Step can be obtained by {@link #getCurrentStep()}.
 * The actor for the current Step is returned by {@link #getCurrentActor()}.
 * </p>
 * <p>
 * To provide flexibility for specific implementations, the Workflow class
 * provides two additional features that enable Workflow participants (
 * <em>i.e.</em>, Workflow subclasses and Step/Task/Decision subclasses) to
 * share context and state information. These two features are
 ...
 * {@link com.ecyrd.jspwiki.i18n.InternationalizationManager} to create
 * language-independent user interface messages. The message argument array is
 * retrieved via {@link #getMessageArguments()}; the first two array elements
 * will always be these: a String representing work flow owner's name, and a
 * String representing the current actor's name. Workflow participants can add
 * to this array by invoking {@link #addMessageArgument(Serializable)}.</li>
 * </ul>
 * <h2>Example</h2>
 * <p>
 ...
 * </p>
 * <ul>
 * <li>Line 1 instantiates the workflow with a sample message key and designated
 * owner Principal, in this case the current wiki user</li>
...
public class Workflow implements Serializable {
	 ...
	 * history, and sets the current step to <code>null</code>. If the Step is a
	 * Decision, it is removed from the DecisionQueue. This method can be called
	 * at any point in the lifecycle prior to completion, but it cannot be
	 * called twice. It finishes by calling the {@link #cleanup()} method to
	 * flush retained objects. If the Workflow had been previously aborted, this
	 ...
	 * {@link #getMessageArguments()}. The object <em>must</em> be an type used
	 * by the {@link java.text.MessageFormat}: String, Date, or Number
	 * (BigDecimal, BigInteger, Byte, Double, Float, Integer, Long, Short). If
	 * the object is not of type String, Number or Date, this method throws an
	 ...
	 * </p>
	 * <ul>
	 * <li>String representing the name of the workflow owner (<em>i.e.</em>,
	 * {@link #getOwner()})</li>
	 * <li>String representing the name of the current actor (<em>i.e.</em>,
	 * {@link #getCurrentActor()}). If the current step is <code>null</code>
	 * because the workflow hasn't started or has already finished, the value of
	 * this argument will be a dash character (<code>-</code>)</li>
	 * </ul>
	 * <p>
	...
	/**
	 * Returns a Step history for this Workflow as a List, chronologically, from
	 * the first Step to the currently executing one. The first step is the
	 * first item in the array. If the Workflow has not started, this method
	 ...
	 * paused, this method throws an IllegalStateException. If any of the Steps
	 * in this Workflow throw a WikiException, the Workflow will abort and
	 * propagate the exception to callers.
	 * 
	 * @throws WikiException
	 *             if the current task's {@link Task#execute()} method throws an
	 *             exception
	 */
	 ...
	 * attribute, for the duration of workflow execution. The passed object can
	 * be anything required by an executing Step, although it <em>should</em> be
	 * serializable. Note that when the workflow completes or aborts, all
	 ...
	 * returns an {@linkplain IllegalStateException}. If any of the Steps in
	 * this Workflow throw a WikiException, the Workflow will abort and
	 * propagate the exception to callers.
	 * 
	 * @throws WikiException
	 *             if the current Step's {@link Step#start()} method throws an
	 ...
	 * calls the {@link #cleanup()} method to flush retained objects. This
	 * method will no-op if it has previously been called.
	 */
	 ...
	 * {@link Step#execute()}. If the <code>execute</code> throws an exception,
	 * this method will propagate the exception immediately to callers without
	 * aborting.
	 * 
	 * @throws WikiException
	 *             if the current Step's {@link Step#start()} method throws an
