 ...
 * </p>
 * <ul>
 * <li><strong>actor</strong>: the Principal responsible for executing the Step;
 * returned by {@link Step#getActor()}.</li>
 * <li><strong>availableOutcomes</strong>: a collection of possible "outcomes,"
 * such as "approve decision" ({@link Outcome#DECISION_APPROVE}),
 * "reassign decision" ({@link Outcome#DECISION_REASSIGN}), "abort step" (
 * {@link Outcome#STEP_ABORT}) and others. The range of possible Outcomes for
 * the Step is returned by {@link Step#getAvailableOutcomes()}; see the Outcome
 ...
 * <li><strong>started</strong> and <strong>completed</strong>: whether the Step
 * has started/finished. These values are returned by {@link Step#isStarted()}
 * and {@link Step#isCompleted()}.</li>
...
public interface Step extends Serializable {
	 ...
	 * <code>STEP_COMPLETE</code> indicates that the instructions executed
	 * completely, without errors; <code>STEP_ABORT</code> indicates that the
	 * Step and its parent Workflow should be aborted (that is, fail silently
	 * without error). If the execution step encounters any errors, it should
	 * throw a WikiException or a subclass.
	 * </p>
	 * <p>
	...
	/**
	 * Returns the message arguments for this Step, typically by delegating to
	 * the parent Workflow's {@link Workflow#getMessageArguments()} method.
	 * 
	 ...
	 * 
	 * @return <code>true</code> if the Step has started; <code>false</code> if
	 *         not.
	 */
	 ...
	 * throws an {@linkplain IllegalStateException}. If the Step cannot be
	 * started because the underlying implementation encounters an error, it the
	 ...
	 * IllegalArgumentException. If the caller attempts to set an Outcome and
	 * the Step has already completed, this method throws an
