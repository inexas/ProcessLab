...
public class WorkflowManager implements WikiEventListener {
	 ...
	 *             if a workflow the automatically assigned ID already exist;
	...
	/**
	 * Returns a collection of finished workflows; that is, those that have
	 ...
	 * require approval. For a given property key, everything after the prefix
	 * denotes the Decision's message key. The property value indicates the
	 * Principal (Role, GroupPrincipal, WikiPrincipal) that must approve the
	 * Decision. For example, if the property key/value pair is
	 * <code>jspwiki.approver.workflow.saveWikiPage=Admin</code>, the Decision's
	 * message key is <code>workflow.saveWikiPage</code>. The Principal
	 * <code>Admin</code> will be resolved via
	 ...
	 * @throws WikiException
	 *             if the message key was not found, or the Principal value
	 *             corresponding to the key could not be resolved
	 */
	 ...
	 * whose {@link Workflow#getOwner()} method returns a Principal also
	 * possessed by the wiki session (see
	 * {@link com.ecyrd.jspwiki.WikiSession#getPrincipals()}). If the wiki
	 * session is not authenticated, this method returns an empty Collection.
	 * 
	 * @param session
	 *            the wiki session
	 ...
	 * and sets its <code>workflowManager</code> and <code>Id</code> properties
	 * if not set.
	 * 
	 * @param workflow
	 *            the workflow to add
	 ...
	 * and moves it to the workflow history list. This method defensively checks
	 * to see if the workflow has not yet been removed.
	 * 
	 * @param workflow
