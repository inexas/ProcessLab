...
/**
 * A simple MBean which does not require an interface class unlike the
 * StandardMBean class. The methods are exposed through a method call, which in
 * turn then calls the methods using the Reflection API.
 * <p>
 ...
 * powerful, but it does not require you to declare two classes (and keep them
 * in sync).
 * 
 * @since 2.6
 */
...
public abstract class SimpleMBean
	...
	protected SimpleMBean() throws NotCompliantMBeanException {
		...
		for(int i = 0; i < oplist.length; i++) {
			...
			if(method == null) {
				throw new NotCompliantMBeanException("Class declares method " + oplist[i]
				        + ", yet does not implement it!");
			}
	...
	/**
	 * Customization hook: Override this to get a description for your MBean. By
	...
	public Object invoke(String arg0, Object[] arg1, String[] arg2)
		...
		throw new ReflectionException(null, "There is no such method " + arg0); // XXX:
																				// Can
																				// you
																				// put
																				// a
	...
	/**
	 * This method must return a list of attributes which are exposed by the
	 * SimpleMBean. If there's a getXXX() method available, it'll be exposed as
	 * a getter, and if there's a setXXX() method available, it'll be exposed as
	 ...
	 * </pre>
	 * 
	 * Also, methods starting with "is" are also recognized as getters (e.g.
	 * <code>public boolean isFoo()</code>.)
	 * 
	...
	/**
	 * This method must return a list of operations which are to be exposed by
	 * the SimpleMBean. Note that using overloaded method names is not supported
	 * - only one will be exposed as a JMX method at random.
	 * 
	 * @return An array of method names that should be exposed as JMX
	 *         operations.
	 */
